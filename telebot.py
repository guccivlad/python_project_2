...
folder_id = '1zsbU9qNQHzWUW1W2LtAf8qVj4IgZl7YA'
...
async def download(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if('.' in input_name):
        '''
        If there is a dot in the entered name, then we assume that this is an extension,
        that is, the name is entered in full (with an extension).
        '''
        input_extension = input_name.split('.')[-1]
        if(input_extension in allowed_extensions):
    ...
    if(extension == ' '):
        for name in files_list:
            if(input_name.lower() in name.lower()):
                applicants[name] = files_list[name]
        if(len(applicants) == 0):
            no_one_file = True
    else:
        file_name = input_name
        file_id = files_list[file_name]
        is_one_file = True
    ...
    if(is_one_file):
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text='We have several files with a similar name. Choose which one you need and write the name in full(just copy the name of the desired file and write /download <copy_name>):')
        number_of_file = 1
        for name in applicants:
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f'{number_of_file}) {name}')
            number_of_file += 1
